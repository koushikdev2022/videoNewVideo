...
const transactionCredit = async (req, res, next) => {
    ...
    const validationRules = [
        ...
        body('id')
            ...
            .isInt()
            .withMessage("transaction_id must be int")
            .custom(async (value) => {
                if (value) {
                    const user = await Transaction.findOne({ where: { id:  payload?.transaction_id,user_id:payload?.user_id } });
                    if (!user) {
